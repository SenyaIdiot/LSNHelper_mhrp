...
local imgui = require 'mimgui' -- òåïåðü ìèìãóé, à íå èìãóé...
...
local newFrame = imgui.OnFrame(
	...
	function(player)
		...
		imgui.Begin(u8'Ïóáëèêàöèÿ oáúÿâëåíèÿ | LSN-Helper '..thisScript().version, nil, imgui.WindowFlags.NoMove + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize)

		imgui.SetCursorPos(imgui.ImVec2(20, 40)); imgui.TextColoredRGB('Îòïðàâèòåëü:'); imgui.SameLine((sizeX - 15) / 2 + 10); imgui.TextColoredRGB('Öåíà:')
		...
		imgui.TextColoredRGB('Ââåäèòå íîâûé òåêñò äëÿ ýòîãî îáúÿâëåíèÿ. Íî íå îñòàâüòå ïîëå ïóñòûì!')

		imgui.SetCursorPos(imgui.ImVec2(20, 180))
		imgui.TextColoredRGB('Âû òàê-æå ìîæåòå îòêëîíèòü îáúÿâëåíèå ñ íàïèñàííîé â ïîëå ïðè÷èíîé è íàæàâ ïîñëå êíîïêó "Îòêëîíèòü".')

		if copying then
			imgui.SetCursorPos(imgui.ImVec2(20, 210))
			imgui.TextColoredRGB('{FFAA00}Òåêñò ñîâïàäàåò îäíîé è òîæå, ñêîïèðîâàíî â ïðîøëîì ðàç. Ïðîâåðüòå, âñ¸ ïðàâèëüíî ëè íàïèñàíî.')
		end
		...
		if imgui.Button(u8'Ïåðåäàòü â /rb', imgui.ImVec2((sizeX - 42) / 2 , 25)) then
			if (u8:decode(str(adInput))) == (nil or '') then
				send('Â òåêñòå ïóñòî, çà÷åì îòïðàâëÿòü?', -1)
			else
		...
		if imgui.Button(u8'Ïîèñê', imgui.ImVec2((sizeX - 42) / 2 , 25)) then
			imgui.OpenPopup(u8'Ïîèñê')
		end

		if imgui.BeginPopupModal(u8'Ïîèñê', _, imgui.WindowFlags.NoMove + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize) then
			local pSize = imgui.ImVec2(770, 340)
			imgui.SetWindowSizeVec2(pSize)
	
			imgui.SetCursorPos(imgui.ImVec2(20, 45))
			imgui.Text(u8'Òåêñò:') imgui.SameLine(70) imgui.TextColoredRGB(adText)
				
			imgui.SetCursorPos(imgui.ImVec2(20, 70)) imgui.Text(u8'Ïîèñê:') imgui.SameLine(70)
			...
			if imgui.Button(u8'Çàêðûòü', imgui.ImVec2(pSize.x - 30, 25)) then
				inputSearch = new.char[256]('')
				imgui.CloseCurrentPopup()
			end
		...
		if imgui.Button(u8'Îïóáëèêîâàòü', imgui.ImVec2((sizeX - 42) / 2 , 25)) then
			...
			elseif Char and Char ~= ('$' or ',' or '/' or '>' or '<' or '-' or '=' or '+' or '_' or "'" or '"') then
				...
				send('Â êîíöå çíàêè ïðåïèíàíèå íå ñòàâèë!')
			end
		end
		imgui.PopStyleColor(3)
		...
		imgui.SameLine((sizeX - 17) / 2 + 10)
		if imgui.Button(u8'Îòêëîíèòü', imgui.ImVec2((sizeX - 42) / 2 , 25)) then
			if (u8:decode(str(adInput))) == (nil or '') then
				send('Âû íè÷åãî íå ââåëè â ïîëå ââîäà', -1)
			else
...
function main()
	...
	send('Ñêðèïò óñïåøíî çàãðóæåíî. Âåðñèÿ: '..thisScript().version)
	...
	sampRegisterChatCommand('verify', function()
		if lockVerify then
			if renderWindow[0] and sampIsDialogActive() then				
				send('Çàêðîéòå äèàëîã è ñíîâà ââîäèòå /verify')
			else
				checkVerify = true
				send('Îáíîâëÿþ '..oldVersion ..' -> '..newVersion..' ...')
				lockVerify = false
			end
		end
	end)
	...
	downloadUrlToFile(update_url, update_path, function(id, status)
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			if newVersion ~= oldVersion then
				send('Åñòü îáíîâëåíèå! Âåðñèÿ: '..newVersion..'. ×òîáû îáíîâèòü ââîäèòå /verify', -1)
				update_state = true
				lockVerify = true
			end
	...
	while true do
		...
		if update_state and checkVerify then
			downloadUrlToFile(script_url, script_path, function(id, status)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					send('Ñêðèïò óñïåøíî îáíîâëåí! Ñåé÷àñ áóäåò ïåðåçàãðóæåí', -1)
					lockFailed = true
					thisScript():reload()
				end
			end)
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
	if id == 1536 and title == '{6333FF}Ïóáëèêàöèÿ îáúÿâëåíèÿ' then
		--local adN = ''
		--adN = ( text:match('%{ffffff%}Îòïðàâèòåëü%: %{7FFF00%}(%w+ %w+)') ):gsub("\n", "")
		adText = ( text:match('%{ffffff%}Òåêñò%:%{7FFF00%} (.*)%{ffffff%}') ):gsub("\n", "")
		adPrice = ( text:match('%{ffffff%}Öåíà%:%{7FFF00%} (.*)%{FFFFFF%}') ):gsub("\n", "")
		renderWindow[0] = true
		autoFocus = true
	...
	elseif id == 1537 and title == '{6333FF}Ïóáëèêàöèÿ îáúÿâëåíèÿ: {ffffff}Ïîäòâåðæäåíèå' and confirm then
...
function sampev.onServerMessage(color, text)
	if color == 2147418282 then
		if text:find('íîâîå îáúÿâëåíèå íà ïðîâåðêó') then
			printStyledString('/edit', 5000, 4)
			send(text)
			return false

		elseif text:find('çàïðîñèë îòêàç íà ïóáëèêàöèþ') then
			send(text)
			return false
	...
	if color == -1616928769 and (text == "Ïîäñêàçêà: ×òîáû îòêðûòü èíâåíòàðü, íàæìèòå 'Y'" or
								 text == "Ïîäñêàçêà: ×òîáû âçàèìîäåéñòâîâàòü ñ áîòîì/èãðîêîì, íàæìèòå 'ïð. êíîïêà ìûøè' + 'H'" or
								 text == "Ïîäñêàçêà: ×òîáû îòêðûòü áàãàæíèê ìàøèíû, íàæìèòå 'ïð. êíîïêà ìûøè' + 'Ïðûæîê'" or
								 text == "Ïîäñêàçêà: Âû ìîæåòå îòêëþ÷èòü ïîìîùü â /mm -> íàñòðîéêè"
		) then return false
	end

	if color == -10059521 and (text:find('Îòêëîíèë îáúÿâëåíèå. Ïðè÷èíà:') or
							   text:find('Íèêòî íå ïîäàâàë îáúÿâëåíèé') or
							   text:find('Äàííîå îáúÿâëåíèå óæå ðåäàêòèðóåò') or
							   text:find('Òîò, êòî ïîäàë îáúÿâëåíèå, ïîêèíóë ñåðâåð')
		) then
		send(text)
		return false
	end

	if color == -1 and text:find('Çà îïóáëèêîâàííîå ñîîáùåíèå âû ïîëó÷èëè') then
		TText = text:match('%{008000%}(%d+)$%{ffffff%}')
		send('Çà îïóáëèêîâàííîå ñîîáùåíèå âû ïîëó÷èëè '..TText..'$ íà âàø áàíê. ñ÷¸ò.')
	...
	if color == -1 and (text:find('Âû îòêëîíèëè îáúÿâëåíèå') or text:find('Òîò, êòî ïîäàë îáúÿâëåíèå, ïîêèíóë ñåðâåð')) then
		send(text)
		return false
	end

	--! ýòîò êîä òîëüêî â ñàìîì êîíöå ôóíêöèè
...
function onScriptTerminate(s, q)
	if s == thisScript() then
		if not lockFailed then
			send('×òî-òî ïîøëî íå òàê ñ ñêðèïòîì... Îòêëþ÷àåì', -1)
		end
		return true
	end
end
